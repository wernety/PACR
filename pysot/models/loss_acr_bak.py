...
class SiamACRLossComputation(object):
    ...
    def compute_targets_for_locations(self, locations, labels, gt_bbox, cls_pos):
        # reg_targets = []
        # locations是一组特殊的值 【【32，40，..32.... 232】【0，0，...，32，32，...232，232】】
        xs, ys = locations[:, 0], locations[:, 1]

        bboxes = gt_bbox
        labels = labels.view(self.cfg.TRAIN.OUTPUT_SIZE**2,-1)  #

        l = xs[:, None] - bboxes[:, 0][None].float()  # 只是将bbox的值做了一个线性变换，表示每个点到
    ...
    def __call__(self, locations, box_cls, box_regression, centerness, labels, reg_targets, cls_pos):
        """
        Arguments:
        ...
        label_cls, reg_targets = self.prepare_targets(locations, labels, reg_targets, cls_pos)  # 这里的 labels 和 reg_targets 都是 GT 的变形（线性变化）
        ...
        pos_inds = torch.nonzero(labels_flatten > 0).squeeze(1)   # pos_index只与BBox有关

        box_regression_flatten = box_regression_flatten[pos_inds]  # 这句的作用是取对应于GT中是目标的点的值
        reg_targets_flatten = reg_targets_flatten[pos_inds]  # 在回归框中选择对应于GT中是目标的点的值
        ...
        if pos_inds.numel() > 0:
            centerness_targets = self.compute_centerness_targets(reg_targets_flatten)  # 计算cen的分数，是用GT进行计算，反映的是在GT中处于中心的程度
