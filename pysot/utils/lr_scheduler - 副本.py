...
class LRScheduler(CosineAnnealingLR):
    def __init__(self, optimizer, last_epoch=-1, epoch=50):
        if 'lr_spaces' not in self.__dict__:
            raise Exception('lr_spaces must be set in "LRSchduler"')
        super(LRScheduler, self).__init__(optimizer,T_max=epoch ,last_epoch=last_epoch)
...
class WarmUPScheduler(LRScheduler):
    def __init__(self, optimizer, warmup, normal, epochs=50, last_epoch=-1):
        ...
        super(WarmUPScheduler, self).__init__(optimizer, last_epoch, epoch=epochs)
